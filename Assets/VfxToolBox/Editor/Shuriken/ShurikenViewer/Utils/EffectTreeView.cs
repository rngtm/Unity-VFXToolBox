namespace VfxToolBox.ShurikenViewer
{
    ...
    public class EffectTreeView : TreeView
    {
        ...
        private static readonly RowFieldAccessor[] fieldAccessorArray = new RowFieldAccessor[]
        {
            new RowFieldAccessor("Sorting Fudge", 100f,
                (rect, ps) =>
                    EditorGUI.LabelField(rect, ps.GetComponent<ParticleSystemRenderer>().sortingFudge.ToString())
            ),
            new RowFieldAccessor("Material", 160f,
                (rect, ps) => EditorGUI.ObjectField(rect, ps.GetComponent<ParticleSystemRenderer>().sharedMaterial,
                    typeof(Material), false)
            ),
            new RowFieldAccessor("Main Texture", 170f,
                (rect, ps) =>
                    EditorGUI.ObjectField(rect, ps.GetComponent<ParticleSystemRenderer>().sharedMaterial?.mainTexture
                        , typeof(Texture2D), false)
            ),
            new RowFieldAccessor("Shader", 320f,
                (rect, ps) => EditorGUI.ObjectField(rect,
                    ps.GetComponent<ParticleSystemRenderer>().sharedMaterial?.shader, typeof(Shader), false)
            ),
            new RowFieldAccessor("Mesh", 130f,
                (rect, ps) =>
                    EditorGUI.ObjectField(rect, ps.GetComponent<ParticleSystemRenderer>().mesh, typeof(Mesh), false)
            ),
        ...
        private void DrawRowColumn(RowGUIArgs args, Rect rect, int columnIndex)
        {
            ...
            if (displayElement.EffectData.ParticleSystem == null)
            {
                return;
            }
